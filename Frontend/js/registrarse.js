async function registrarse(e) {
  ...
  try {
    const { ok, data } = await registrarUsuario(nombre, usuario, correo, password);
    if (ok) {
      await Swal.fire({ icon: 'success', title: '¡Cuenta creada!', text: 'Ahora puedes iniciar sesión.', confirmButtonColor: '#007ABF' });
      window.location.href = '../index.html';
    } else {
      Swal.fire({ icon: 'error', title: 'Error al registrar', text: data.mensaje, confirmButtonColor: '#007ABF' });
    }
  } catch (err) {
