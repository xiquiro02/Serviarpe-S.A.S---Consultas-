...
async function cargarUsuarios() {
  ...
  data.forEach(function (u) {
    ...
    tr.innerHTML =
      ...
      '<td><div class="user-cell"><span class="avatar-icon">👤</span><span class="nombre-usuario">' + u.nombre + '</span></div></td>' +
      '<td>' + u.correo + '</td>' +
...
async function abrirModalEditar(btn) {
  ...
  const { ok, data: u } = await getUsuarioById(usuarioEditandoId);
  if (!ok || !u) return;
...
async function guardarUsuario() {
  ...
  const { ok, data } = esEdicion
    ? await updateUsuario(usuarioEditandoId, nombre, correo)
    : await createUsuario(nombre, usuario, correo, pass, rol);
...
function confirmarEliminar(btn) {
  ...
  Swal.fire({
    ...
    const { ok, data } = await deleteUsuario(id);
    if (!ok) {
      Swal.fire({ icon: 'error', title: 'Error', text: data.mensaje, confirmButtonColor: '#007ABF' });
      return;
    }
...
function cambiarRol(btn) {
  ...
  const nuevoRol   = badge.dataset.rol === 'administrador' ? 'empleado' : 'administrador';
  ...
  Swal.fire({
    ...
    confirmButtonColor: '#007ABF', cancelButtonColor: '#6c757d',
    confirmButtonText: 'Sí, cambiar', cancelButtonText: 'Cancelar'
  }).then(async result => {
    if (!result.isConfirmed) return;
    const { ok } = await updateRolUsuario(id, nuevoRol);
    if (!ok) return;
