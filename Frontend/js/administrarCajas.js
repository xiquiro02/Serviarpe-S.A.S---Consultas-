...
async function guardarCaja() {
  ...
  const { ok, data } = esEdicion
    ? await updateCaja(cajaEditandoId, numero, ubicacion)
    : await createCaja(numero, ubicacion);
...
function confirmarEliminar(btn) {
  ...
  Swal.fire({
    title: '¿Eliminar esta caja?', text: 'Esta acción no se puede deshacer.',
    icon: 'warning', showCancelButton: true,
    confirmButtonColor: '#e74c3c', cancelButtonColor: '#6c757d',
    confirmButtonText: 'Sí, eliminar', cancelButtonText: 'Cancelar'
  }).then(async result => {
    if (!result.isConfirmed) return;
    const { ok } = await deleteCaja(id);
    if (!ok) return;
    cargarCajas();
